...
NES_data_glass <- read.csv("reports/combined/outputs_absolute_tss_1000_nes.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_glass), ]
...
FDR_data_glass <- read.csv("reports/combined/outputs_absolute_tss_1000_fdr.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_glass), ]
...
NES_data$Level <- ifelse(grepl('\\.y', NES_data$Level), 'TSS', 'Gene')
FDR_data$Level <- ifelse(grepl('\\.y', FDR_data$Level), 'TSS', 'Gene')
...
NES_data$Level<- relevel(NES_data$Level, "Gene")
FDR_data$Level<- relevel(FDR_data$Level, "Gene")
...
pdf(paste("analysis/gsea_results/gsea_tss.pdf", sep=""))
