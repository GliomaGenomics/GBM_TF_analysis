...
patients_glass<-scan("patient_lists/gbm_idhwt_rt_tmz_local.txt", what = character())
patients_stead<-append(patients_stead,"Gene")
patients_glass<-append(patients_glass,"Gene")
NES_data_stead <- read.csv("reports/combined/outputs_absolute_2000_nes.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_stead), ]
NES_data_glass <- read.csv("reports/combined/outputs_absolute_5000_nes.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_glass), ]
FDR_data_stead <- read.csv("reports/combined/outputs_absolute_2000_fdr.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_stead), ]
FDR_data_glass <- read.csv("reports/combined/outputs_absolute_5000_fdr.txt", stringsAsFactors = T, sep='\t',na.strings=c("---"))[, as.vector(patients_glass), ]
...
NES_data<-pivot_longer(NES_data, cols = -Gene, names_to = "Level", values_to = "NES")
FDR_data<-pivot_longer(FDR_data, cols = -Gene, names_to = "Level", values_to = "FDR")
NES_data$Level <- ifelse(grepl('\\.y', NES_data$Level), '2kb', '5kb')
FDR_data$Level <- ifelse(grepl('\\.y', FDR_data$Level), '2kb', '5kb')
...
NES_data<-NES_data[ , c('NES', 'Molecule', 'Level')] 
FDR_data<-rename(FDR_data, Molecule = Gene)
FDR_data<-FDR_data[ , c('FDR', 'Molecule', 'Level')] 

print(NES_data)
#NES_data<-NES_data[order(NES_data$Level),]
#FDR_data<-FDR_data[order(FDR_data$Level),]
...
NES_data$Level<-as.factor(NES_data$Level)
FDR_data$Level<-as.factor(FDR_data$Level)
NES_data$Level<- relevel(NES_data$Level, "2kb")
FDR_data$Level<- relevel(FDR_data$Level, "2kb")
...
#print(typeof(NES_data$Level))
#print(NES_data$Molecule)
#print(NES_data$NES)
#print(NES_data$Level)
print(NES_data$NES)


pdf(paste("analysis/gsea_results/gsea_2-5kb.pdf", sep=""))

# NES PLOT STARTS HERE

p1_NES <- ggplot(NES_data, aes(Molecule, NES, fill=Level)) + geom_split_violin(scale = "width") 
...
p1_FDR <- ggplot(FDR_data, aes(Molecule, FDR, fill=Level)) + geom_split_violin(scale = "width", ylim=c(0,2), show.legend = FALSE)
